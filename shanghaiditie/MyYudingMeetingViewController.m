//
//  MyYudingMeetingViewController.m
//  shanghaiditie
//
...
@interface MyYudingMeetingViewController ()
{
...
@implementation MyYudingMeetingViewController
...
- (void)viewDidLoad {
    ...
    [self getYudingMeeting:pageIndex pageSize:10];
}
...
////获取我的会议列表
- (void)getYudingMeeting:(NSInteger)pageindex pageSize:(NSInteger)pagesize
{
    ...
    NSString *str1 = [NSString stringWithFormat:@"{\"action\":\"BGS_HY_OurMeetingList_ByPage\","];
...
- (void)requestFinished:(ASIHTTPRequest *)_request {
    ...
    if(_request.tag == 501)
    {
        if([[result objectForKey:@"State"] isEqualToString:@"Sucess"]){
            ...
            totalPageNumber = [[[result objectForKey:@"Message"] objectForKey:@"count"] intValue];
...
//            [MBProgressHUD hideHUDForView:self.view];
//            [MBProgressHUD showSuccess:@"没有预定会议"];
...
- (void)requestFailed:(ASIHTTPRequest *)_request {
    [MBProgressHUD hideHUDForView:self.view];
    [MBProgressHUD showError:@"获取我预定的会议失败，请检查网络！"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ([[dic objectForKey:@"ExamineState"] intValue]) {
        case 1:
            cell.labelForStatu.text = @"未审核";
            break;
        case 2:
            cell.labelForStatu.text = @"审核通过";
            break;
        case 3:
            cell.labelForStatu.text = @"审核不通过";
            break;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MyYudingMeetingDetailViewController * myYudingMeetingDetailViewController = [storyboard instantiateViewControllerWithIdentifier:@"myYudingMeetingDetailViewController"];
    myYudingMeetingDetailViewController.strForMeetingId = [[NSString alloc]initWithString:[dic objectForKey:@"Id"]];
    myYudingMeetingDetailViewController.dicForMeetingInfo = [[NSDictionary alloc]initWithDictionary:dic];
    [self.navigationController pushViewController:myYudingMeetingDetailViewController animated:YES];
}
